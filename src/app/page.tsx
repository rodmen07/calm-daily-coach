        ...
        <section className="panel">
          <form className="space-y-4" onSubmit={handleGeneratePlan}>
            ...
            <div>
              ...
              <p className="dose-hint">
                Pick a focus area above, or scan the full set of areas the app can coach you through.
              </p>
              <ul className="category-grid" role="list" aria-label="Improvement categories">
                {FOCUS_AREAS.map((area) => (
                  <li
                    key={area}
                    className={`category-chip ${focus === area ? "is-selected" : ""}`}
                  >
                    {area}
                  </li>
                ))}
